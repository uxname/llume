...
describe('AI Functions and Routing', () => {
    ...
    it('executes weather function', async () => {
        const response = await weather.execute({ city: 'Minsk', date: '1 января 2023' });
    ...
    it('throws error for unknown AI function', () => {
        ...
        expect(() => getAiFunctionByName('Unknown')).toThrow('Unknown AI function name');
    });
});
...
async function listAllReferences(accumulator: string[], filepath: string): Promise<string[]> {
    const result = await detectCodeReferences(filepath, JSON.stringify(accumulator));
