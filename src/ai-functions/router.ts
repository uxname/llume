...
const agentRouterResponseSchema = z.union([
  ...
  z.object({
    ...
    errors: z
      .array(z.string())
      .describe(
        "Список ошибок, если есть, например если подходящего агента нет",
      ),
  }),
]);
...
export class AgentRouter extends AiFunction<AgentRouterResponse> {
  constructor(aiExecutionEngine?: AiExecutionEngineBase) {
    super({
      ...
      prompt:
        "Твоя задача выбрать наиболее подходящего агента для выполнения задачи. Вот задача: {task}, вот список доступных агентов: {agents}",
