export class LlmHelper {
	...
	static sanitizeLLMJsonResponse(response: string): string {
		...
		const strategies: Array<() => string | null> = [
			() => LlmHelper.extractJsonBlock(response),
			() => LlmHelper.extractGenericCodeBlock(response),
			() => LlmHelper.extractByBraceBounds(response),
		];
	...
	private static extractByBraceBounds(response: string): string | null {
		...
		const starts = LlmHelper.findStartIndices(response);
